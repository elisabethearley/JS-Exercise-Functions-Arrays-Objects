...
makePersonObject(10, 'liz', 'lizrule@gmail.com');
...
function getLastCarInfo(array) {
  ...
  //return 'This is a ' + array[49].car_make + array[49].car_model;
}
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(array, i) {
  return `This is a ${array[i - 1].car_make} ${array[i - 1].car_model}`;
}
...
function getGermanCars(array) {
  let german = [];
  let germanMade = [`Audi`, `Mercedes-Benz`, `BMW`, `Volkswagen`];

  for (let i = 0; i < array.length; i++) {
    if (germanMade.includes(array[i].car_make)) {
      german.push(array[i])
    }
  }
  return german;
...
function carMaker(number) {
  const odom = {
    ...
    drive: function(distance) {
      odom.odometer = odom.odometer + distance;
